...
void ZeroCheck_cal(void)
{
	for (int i=1;i<=4;i++)
	{
		float decode_angle = ZeroCheck(&(ZeroCheck_Yaw_Steering[Steering_ID_list[i]]),CAN2_Motors[Steering_ID_list[i]].totalAngle,Position);
...
void ZeroCheck_Init(void)
{
	for (int i=1;i<=4;i++)
	{
		...
		ZeroCheck_Yaw_Steering[Steering_ID_list[i]].LastValue = CAN2_Motors[Steering_ID_list[i]].totalAngle;
